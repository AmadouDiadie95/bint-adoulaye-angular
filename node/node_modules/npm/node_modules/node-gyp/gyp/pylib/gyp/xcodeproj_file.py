...
class XCTarget(XCRemoteObject):
    ...
    #
    # Setting a "name" property at instantiation may also affect "product_name",
    ...
    def __init__(
        ...
        # property was supplied, set "product_name" if it is not present.  Also set
        # the "PRODUCT_NAME" build setting in each configuration, but only if
        # the setting is not present in any build configuration.
        if "name" in self._properties:
            if "product_name" not in self._properties:
                self.SetProperty("product_name", self._properties["name"])

        if "product_name" in self._properties:
            if "buildConfigurationList" in self._properties:
                configs = self._properties["buildConfigurationList"]
                if configs.HasBuildSetting("PRODUCT_NAME") == 0:
                    configs.SetBuildSetting(
                        "PRODUCT_NAME", self._properties["product_name"]
                    )
...
class PBXNativeTarget(XCTarget):
    ...
    def __init__(
        ...
        if (
            "product_name" in self._properties
            ...
            if products_group is not None:
                ...
                if force_extension is not None:
                    ...
                    if filetype.startswith("compiled.mach-o.executable"):
                        ...
                        product_name += suffix
                        suffix = ""
                        self.SetProperty("product_name", product_name)
